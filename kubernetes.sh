...
# default
servername=user
kubernetesServername="$servername"-server # *.server.yaml.metadata.name
podName="$servername"-server # *.deployment.yaml.metadata.name
serverPort=9801
targetPort=9801
...
if [ "$deploymentStatus" != 0 ]; then # 有正在运行的pods重启
